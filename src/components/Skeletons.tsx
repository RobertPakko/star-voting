 ...
 *
 * The padding is the tap target's rather than the star's: each star is a
 * 20px icon in a button padded to 26 by 32, and it is the button that sets
 * how tall an option's row is. Drawn as bare icons the rows came up 12px
 * short each, so the card grew as the ballot landed in it.
 */
function StarsShape() {
  return (
    <Group gap={6} px={3} py={6} wrap="nowrap" style={{ flex: 'none' }}>
 ...
 *
 * The card itself is the caller's, because one of the two callers has
 * something real to put in it; see `QuestionSkeleton`.
 */
 ...
 *
 * **The strip is handed in and drawn for real**, in the card and in the place
 * the ballot draws it, rather than being one more shape. It belongs to the
 * poll like the heading above it does — the same questions in the same order,
 * with the badge already moved to the one being opened — and only happens to
 * live inside the card a crossing replaces. Standing in for it was a strip
 * that greyed out and came back at the exact moment a reader was using it to
 * navigate, which is the thing this whole stand-in exists to prevent, and it
 * put the way out of the question behind the wait: the links are live while
 * the ballot loads, so a reader who crossed to the wrong question can cross
 * straight back.
 *
 * It is therefore outside the `Loading` wrapper, which hides what it holds
 * from screen readers. Real links in there would be links nothing could reach
 * — announced to nobody and still in the tab order.
 */
export function QuestionSkeleton({ rows = 3, strip }: { rows?: number; strip?: ReactNode }) {
  return (
    <Card withBorder>
      <Stack gap="sm">
        {strip}
...
/**
 * The roster of everyone in an invite poll, under the heading the page has
 * already drawn: a row per person, with what they have done so far beside
 * their address.
 *
 * The one stand-in here that guesses at nothing. Every other shape in this
 * file is drawn before its read has landed and has to claim the common case;
 * this card sits inside a page that has already read the poll, so how many
 * people are in it, whether their standing is shown at all, and whether the
 * reader is the creator who manages the list are all known facts by the time
 * it is drawn. A poll with nobody invited yet draws no rows, which is what
 * the roster itself does.
 *
 * See Respondents, whose card this is.
 */
export function RosterSkeleton({
  ...
  /** How many people are in the poll: `invited_count`, off the same status. */
  rows: number
  /** Whether the poll says what each of them has done; `show_voters`. */
  badges?: boolean
  /** The creator's `×` at the end of every row. */
  controls?: boolean
  /** And their box for inviting somebody else, which a finished poll drops. */
  invite?: boolean
}) {
  return (
    <Loading>
        ...
        <Stack gap="xs">
          {Array.from({ length: rows }, (_, i) => (
            <Group key={i} justify="space-between" wrap="nowrap" gap="xs">
              <Skeleton
                height={bar.line}
                width={ADDRESSES[i % ADDRESSES.length]}
                maw="100%"
                radius="sm"
              />
              <Group gap="xs" wrap="nowrap">
                {badges && <Skeleton height={18} width={62} radius="xl" />}
                {controls && <Skeleton height={28} width={28} radius="md" />}
              </Group>
            </Group>
          ))}
          {invite && (
            <Group gap="xs" wrap="nowrap" mt={4}>
              ...
              <Skeleton height={control} width={72} radius="md" />
            </Group>
          )}
        </Stack>
      </Card>
...
/**
 * Email addresses are all different lengths, and a column of identical bars
 * reads as a table of one repeated value rather than as a list of people.
 */
const ADDRESSES = [196, 232, 168, 244, 184, 216]
