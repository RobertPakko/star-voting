-- Adds the five-star tie-break to the runoff round.
--
-- The score round already settles a tie on head-to-head preference and then
-- on five-star counts. The runoff settled on preference and then total score,
-- and gave up. Two finalists level on both is not rare in a poll of five
-- people, so the same five-star count now gets one more chance to name a
-- winner before the result is reported as a genuine tie. Using the one rule
-- in both rounds also means there is one fewer thing to explain.
--
-- The counts ride along in the runoff payload so the result can show its
-- work, the way the score round's tie-breaks already do.
