-- Fixes "function max(uuid) does not exist": get_poll_results() used
-- max(candidate_id) filter (...) to pick finalists, but Postgres has no
-- built-in max() aggregate for uuid. Replaced with scalar subqueries
-- against the same ranked CTE.
