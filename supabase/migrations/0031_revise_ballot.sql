...
-- ---------------------------------------------------------------------------
-- The two places the app learns whether the results are out
--
-- Both replaced here for one reason: each carried its own copy of the rule,
-- and a revision is refused by `poll_results_revealed()` while the button
-- offering it is drawn from whatever these two say. Two copies that agree
-- today are a screen offering something the database will refuse tomorrow, so
-- there is now one copy and these read it.
-- ---------------------------------------------------------------------------
